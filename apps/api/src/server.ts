...
import { StoreError, type RunStore, type WorkflowStore } from './stores.js';
...
export function createApi(options: ApiOptions): { server: Server; runs: RunManager } {
  ...
  async function handle(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    switch (route) {
      ...
      case 'GET /workflows':
        return send(res, 200, await options.workflows.list());
      ...
      case 'PUT /workflows/:id': {
        ...
        return send(res, 200, await options.workflows.put(document));
      }
      ...
      case 'POST /workflows/:id/runs': {
        ...
        const record = await runs.start(document, body && 'input' in body ? toEnvelope(body.input) : undefined);
